...
MainWindow::on_btnSetHeader_clicked();
ui->spinBox->setValue(8);
ui->tableInfo->setRowCount(ui->spinBox->value());
    ...
    MainWindow::on_btnAutoHeght_clicked();
    MainWindow::on_btnAutoWidth_clicked();  //自定义
    MainWindow::on_checkBoxRowColor_clicked(Qt::Checked);
}
...
void MainWindow::on_checkBoxTabEditable_clicked(bool checked)
{ //设置编辑模式
    if (checked)
//双击或获取焦点后单击，进入编辑状态
...
void MainWindow::on_btnReadToEdit_clicked()
{
    ...
    for(int i=0;i<ui->tableInfo->rowCount();i++)
    {
        ...
        cellItem =ui->tableInfo->item(i,ui->tableInfo->columnCount()-1);
...
{
    ui->tableInfo->setRowCount(ui->spinBox->value());
    ui->tableInfo->setAlternatingRowColors(ui->checkBoxRowColor->isChecked());
}
...
{//双击时进入编辑状态，加入一个QComboBox进行选择
   ...
   item->setIcon(icon);
   ui->tableInfo->setItem(row,column,item);//为单元格设置Item
}
   ...
   for (i=0;i<ui->tableInfo->rowCount();i++)
    {
      ...
      ui->tableInfo->setCellWidget(i,colNation,cBoxNation); //为单元格设置一个控件，但是这个控件并未起到编辑作用
   }
